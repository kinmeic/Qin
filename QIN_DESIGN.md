...
- Linux/macOS 下通过 `sudo qin init` 启动时进入系统配置域，与 `sudo qin` 的加载规则一致；输出中明确标注“系统级配置”。
...
`sudo qin` 仍然支持，但定义为“整个 Agent 运行于系统管理员配置域”：

- 优先读取 `/etc/qin/config.toml` 和系统数据库。
- 如果确实要使用原用户配置，必须显式 `--user-profile`，并按 `SUDO_UID` 定位，避免依赖被 sudo 改写的 `$HOME`。
- 不能让 root 向普通用户数据库写入 root-owned 文件；打开后应保持原属主，或使用独立系统数据库。
